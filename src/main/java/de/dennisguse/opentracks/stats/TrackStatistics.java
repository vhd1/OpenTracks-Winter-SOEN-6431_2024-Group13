...
public class TrackStatistics {
    ...
    public TrackStatistics(TrackStatistics other) {
        ...
        altitudeExtremities.set(other.altitudeExtremities.getMin(),
                other.altitudeExtremities.getMax());
    ...
    @VisibleForTesting
    public TrackStatistics(String startTime, String stopTime, double totalDistance_m, int
            totalTime_s, int movingTime_s, float maxSpeed_mps, Float totalAltitudeGain_m, Float
                                   totalAltitudeLoss_m) {
    ...
    public TrackStatistics(Instant startTime, Instant stopTime, Distance totalDistance, Duration
            totalTime,
                           Duration movingTime, Speed maxSpeed, Float totalAltitudeGain_m,
                           Float totalAltitudeLoss_m, HeartRate avgHeartRate, boolean isIdle, Float
                                   slope) {
...
// Important to do this before total time is updated
                avgHeartRate = HeartRate.of(
                        (totalTime.getSeconds() * avgHeartRate.getBPM() +
                                other.totalTime.getSeconds() * other.avgHeartRate.getBPM())
...
// Time must be monotonically increasing, but we might have events at the same point in time (BLE and GPS)
    ...
    public int calculateSlopePercentage() {
        if (totalDistance.isZero()) {
            return 0;
        }
        return (int) ((getMaxAltitude() - getMinAltitude()) / totalDistance.distance_m());
    }
     ...
     * If you need to calculate the proper total time, use {@link #getStartTime} with the current
     time.
     */
     ...
     * This calculation only takes into account the displacement until the last point that was
     accounted for in statistics.
     */
     ...
     * This is calculated from the smoothed altitude, so this can actually be less than the current
     altitude.
     */
